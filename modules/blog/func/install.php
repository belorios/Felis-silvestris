...
#		"Categories" => DB_PREFIX . "{$moduleName}Categories",		##NOT YET IMPLEMENTED
		...
		"Tags" 	 	 => DB_PREFIX . "{$moduleName}Tags",
		"TagsPosts"  => DB_PREFIX . "{$moduleName}TagsPosts",
	);
	...
	$tableUsers = DB_PREFIX . "Users";
		...
		$sqlTableCreate['Tags'] = "
			CREATE TABLE IF NOT EXISTS $tables[Tags] (
			  ...
			  -- Primary key(s)
			  idTags BIGINT AUTO_INCREMENT NOT NULL PRIMARY KEY,
		...
		$sqlTableCreate['TagsPosts'] = "
			CREATE TABLE IF NOT EXISTS $tables[TagsPosts] (
				
				-- Foreign key(s)
				idTags    BIGINT NOT NULL,
				idPosts   BIGINT NOT NULL,
				
				FOREIGN KEY (idTags)
					REFERENCES $tables[Tags](idTags)
					ON UPDATE CASCADE ON DELETE CASCADE,
				FOREIGN KEY (idPosts)
					REFERENCES $tables[Posts](idPosts)
					ON UPDATE CASCADE ON DELETE CASCADE,
				PRIMARY KEY (idTags, idPosts)
		...
		if ($_POST['dummyData'] == '1') {
			...
			//Creates som tags
			$sqlCreateData['Tags'] = "
				INSERT INTO $tables[Tags] (tagname) VALUES 
				('mode'), 
				('teknik'),
				('konst'),
				('flyg'),
			...
			//Links tags to posts
			$sqlCreateData['TagsPosts'] = "
				INSERT INTO $tables[TagsPosts] (idPosts, idTags) VALUES 
				(1,1), 
				(1,2),
				(2,2),
				(3,3),
